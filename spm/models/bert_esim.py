...
@Model.register("bert-esim")
class BertESIM(Model):
    """
    ...
    by Chen et al., 2017.
    BertESIM is the ESIM model using BERT as embedding layer.
    ...
    def forward(self,  # type: ignore
        ...
        tokens : Dict[str, torch.LongTensor]
            From a ``TextField``
            Concatenation of premise tokens and hypothesis tokens, joined by a [SEP] token.
        ...
        # NOTE: do not pass `bert_offsets` to the bert embedder here, so that embeded_tokens
        # has the same shape with token_type_ids and mask
        mask = (tokens['bert'] != 0).float()
        token_type_ids = tokens['bert-type-ids'].float()

        premise_mask = 1 - ((1-mask) + token_type_ids)
        hypothesis_mask = token_type_ids
        embedded_premise = embedded_tokens * premise_mask.unsqueeze(-1)
        embedded_hypothesis = embedded_tokens * hypothesis_mask.unsqueeze(-1)

        # TODO: this kind of mask will severely deteriorate performence. Try to use another implementation
        ...
        encoded_premise = self._encoder(embedded_premise, mask=None)
        encoded_premise = encoded_premise * premise_mask.unsqueeze(-1)
        encoded_hypothesis = self._encoder(embedded_hypothesis, mask=None)
        encoded_hypothesis = encoded_hypothesis * hypothesis_mask.unsqueeze(-1)
        ...
        v_ai = self._inference_encoder(
            projected_enhanced_premise, mask=None)
        v_ai = v_ai * premise_mask.unsqueeze(-1)
        v_bi = self._inference_encoder(
            projected_enhanced_hypothesis, mask=None)
        v_bi = v_bi * hypothesis_mask.unsqueeze(-1)
