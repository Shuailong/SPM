...
class StarTransformerEncoder(nn.Module, FromParams):
    ...
    def __init__(self, hidden_size, num_layers, num_head, head_dim, dropout=0.1, max_len=None):
        ...
        self.norm = nn.ModuleList([nn.LayerNorm(hidden_size)
    ...
    def forward(self, data, mask):
        ...
        smask = torch.cat([torch.zeros(B, 1).to(mask), mask], 1)  # B L+1
        ...
        ex_mask = mask[:, None, :, None].expand(B, H, L, 1)  # B H L 1
        r_embs = embs.view(B, H, 1, L)  # B H 1 L
        for i in range(self.num_layers):
            ax = torch.cat([r_embs, relay.expand(B, H, 1, L)], 2)  # B H 2 L
            nodes = nodes + \
                F.leaky_relu(self.ring_att[i](
                    norm_func(self.norm[i], nodes), ax=ax))
            relay = F.leaky_relu(self.star_att[i](
                relay, torch.cat([relay, nodes], 2), smask))
...
class MSA1(nn.Module):
    def __init__(self, nhid, nhead=10, head_dim=10, dropout=0.1):
        ...
        self.nhead, self.head_dim, self.unfold_size = nhead, head_dim, 3

    def forward(self, x, ax=None):
        # x: B, H, L, 1, ax : B, H, X, L append features
        nhead, head_dim, unfold_size = self.nhead, self.head_dim, self.unfold_size
        B, H, L, _ = x.shape
        ...
        if ax is not None:
            ...
            v = torch.cat([v, av], 3)
        alphas = self.drop(
            F.softmax((q * k).sum(2, keepdim=True) / NP.sqrt(head_dim), 3))  # B N L 1 U
...
class MSA2(nn.Module):
    ...
    def forward(self, x, y, mask=None):
        ...
        nhead, head_dim = self.nhead, self.head_dim
        B, H, L, _ = y.shape
        ...
        v = k.view(B, nhead, head_dim, L).permute(
            0, 1, 3, 2)  # B, H, L, 1 -> B, N, L, h
        pre_a = torch.matmul(q, k) / NP.sqrt(head_dim)
        if mask is not None:
            pre_a = pre_a.masked_fill(mask[:, None, None, :], -float('inf'))
        alphas = self.drop(F.softmax(pre_a, 3))  # B, N, 1, L
        att = torch.matmul(alphas, v).view(
            B, -1, 1, 1)  # B, N, 1, h -> B, N*h, 1, 1
        return self.WO(att)
